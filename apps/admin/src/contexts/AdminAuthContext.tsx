...
export function AdminAuthProvider({ children }: { children: ReactNode }) {
  ...
  const client = createCeerionMailClient({
    ...
    headers: user ? { Authorization: `Bearer ${localStorage.getItem('admin_token')}` } : {}
  })
  ...
  const verifyAdminToken = async (_token: string) => {
    try {
      ...
      const mockAdmin: AdminUser = {
        ...
        role: 'super_admin',
        name: 'Admin User',
        permissions: ['users:read', 'users:write', 'quarantine:manage', 'deliverability:read', 'policies:write', 'audit:read']
      }
      setUser(mockAdmin)
    } catch (error) {
  ...
  return (
    <AdminAuthContext.Provider value={{ user, isLoading, login, logout, client }}>
      {children}
