...
const mockEmailPolicies: EmailPolicy[] = [
  {
    ...
    description:
      "Automatically block emails from domains known to distribute malware",
    type: "domain_filter",
    category: "security",
    status: "active",
    priority: 1,
  ...
  },
  {
    ...
    description:
      "Quarantine emails with spam score above 8.0 for manual review",
    type: "spam_filter",
    category: "security",
    status: "active",
    priority: 2,
...
const mockEmailPolicyStats: EmailPolicyStats = {
  ...
  applicationStats: [
    {
      policyId: "policy-005",
      policyName: "Compliance Archive",
      appliedCount: 8956,
    },
    {
      policyId: "policy-002",
      policyName: "Quarantine High Spam Score",
      appliedCount: 234,
    },
    {
      policyId: "policy-004",
      policyName: "Large Attachment Warning",
      appliedCount: 123,
    },
    {
      policyId: "policy-003",
      policyName: "Executive Email Protection",
      appliedCount: 67,
    },
    {
      policyId: "policy-001",
      policyName: "Block Known Malware Domains",
      appliedCount: 45,
    },
  ],
};
...
// API Functions
export const getEmailPolicies = async (
  ...
  if (filters.type) {
    filteredPolicies = filteredPolicies.filter(
      (policy) => policy.type === filters.type,
    );
  }
...
export const toggleEmailPolicyStatus = async (
  id: string,
): Promise<EmailPolicy> => {
  await new Promise((resolve) => setTimeout(resolve, 300));
...
export const deleteEmailPolicy = async (_id: string): Promise<void> => {
  await new Promise((resolve) => setTimeout(resolve, 300));
