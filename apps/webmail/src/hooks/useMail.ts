...
function mapApiMessage(apiMessage: any): Message {
  return {
    ...
    from: {
      name:
        apiMessage.from?.split("<")[0]?.trim() || apiMessage.from || "Unknown",
      email: apiMessage.from?.match(/<(.+)>/)?.[1] || apiMessage.from || "",
    },
    to: Array.isArray(apiMessage.to)
      ? apiMessage.to.map((addr: string) => ({
    ...
    cc: apiMessage.cc
      ? Array.isArray(apiMessage.cc)
        ? apiMessage.cc.map((addr: string) => ({
            name: addr.split("<")[0]?.trim() || addr,
            email: addr.match(/<(.+)>/)?.[1] || addr,
          }))
        : []
...
export function useMail() {
  ...
  const composeFromMessage = async (
    messageId: string,
    ...
    try {
      const { data, error } = await client.mail.composeFromMessage({
        messageId,
        action,
        ...options,
      });
      ...
      if (data && !error) {
        ...
        throw new Error(`Failed to compose ${action}`);
      }
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : `Failed to compose ${action}`;
      toast.error(errorMessage);
      throw err;
    }
  };

  // Convenience methods for reply actions
  const reply = (messageId: string, selectedTextHtml?: string) =>
    composeFromMessage(messageId, "reply", { selectedTextHtml });

  const replyAll = (messageId: string, selectedTextHtml?: string) =>
    composeFromMessage(messageId, "replyAll", { selectedTextHtml });
  ...
  const forward = (
    ...
    composeFromMessage(messageId, "forward", {
      includeAttachments,
      selectedTextHtml,
    });
  ...
  const markMessages = async (
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : `Failed to ${action} messages`;
      toast.error(errorMessage);
      throw err;
    }
  };
  ...
  const snoozeMessages = async (messageIds: string[], snoozeUntil: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to snooze messages";
      toast.error(errorMessage);
      throw err;
    }
  };
