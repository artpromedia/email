...
const authPlugin: FastifyPluginAsync<AuthPluginOptions> = async (fastify, options) => {
  ...
  fastify.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (request.url.startsWith('/health') || request.url.startsWith('/readiness')) {
      return;
    }

    // Skip auth for login/register endpoints
    if (request.url.startsWith('/auth/login') || request.url.startsWith('/auth/register')) {
      return;
    }

    // Skip auth for OpenAPI docs
    if (request.url.startsWith('/docs') || request.url.startsWith('/openapi')) {
      return;
    }
...
export const requireMFA = async (request: FastifyRequest, reply: FastifyReply) => {
  if (!request.user) {
    return reply.code(401).send({
      error: 'Unauthorized',
      message: 'Authentication required',
...
export const createRateLimiter = (options: {
  ...
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    reply.header('X-Rate-Limit-Remaining', Math.max(0, options.max - current).toString());
  };
};
