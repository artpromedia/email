...
const mailRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Body: SendMailRequest }>('/send', {
    schema: {
      body: {
        type: 'object',
        required: ['to', 'subject'],
    ...
    const { to, cc, bcc, subject, body, htmlBody, attachments, priority = 'normal', scheduleAt } = request.body;
    ...
    return await telemetry.withSpan('mail-send-operation', async (span) => {
      ...
      try {
        ...
        const messageSize = Buffer.byteLength(body || '', 'utf8') + 
          (attachments?.reduce((sum: number, att: any) => sum + (att.size || 0), 0) || 0);

        // Check size limits
        if (messageSize > 52428800) { // 50MB
  ...
  fastify.get<{ 
    ...
    return await telemetry.withSpan('mail-list-operation', async (span) => {
      try {
        const messages = await mailService.getUserMessages(user.sub, folder, limit, offset);
  ...
  fastify.get<{ Params: { messageId: string } }>('/message/:messageId', {
    schema: {
      params: {
        type: 'object',
        properties: {
  ...
  // Search messages
  fastify.post<{ Body: MailSearchRequest }>('/search', {
    schema: {
      body: {
        type: 'object',
        properties: {
    ...
    return await telemetry.withSpan('mail-search-operation', async (span) => {
      try {
        const result = await mailService.searchMessages(user.sub, searchParams);
        telemetry.recordMailOperation('read', 'success', user.sub);
        return result;
      } catch (error) {
        telemetry.recordMailOperation('read', 'failure', user.sub);
