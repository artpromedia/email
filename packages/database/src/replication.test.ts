...
describe('ReplicationConfig', () => {
  ...
  it('should throw if DATABASE_URL is not set', () => {
    ...
    expect(() => getReplicationConfig()).toThrow('Missing required environment variable: DATABASE_URL');
  });
  ...
  it('should parse replica URLs from environment', () => {
    ...
    process.env['DATABASE_REPLICA_URLS'] = 'postgresql://user:pass@replica1:5432/db,postgresql://user:pass@replica2:5432/db';
...
describe('ReplicationClient', () => {
  ...
  describe('read and write operations', () => {
    it('should execute read operations', async () => {
      await client.initialize();
      const result = await client.read(async (db) => {
        // Mock read operation
        return { data: 'test' };
      });
    ...
    it('should execute write operations', async () => {
      await client.initialize();
      const result = await client.write(async (db) => {
