...
export class Telemetry {
  ...
  // Tracing utilities
  public withSpan<T>(name: string, fn: (span: any) => Promise<T> | T): Promise<T> {
  ...
  public recordAuthAttempt(method: string, status: 'success' | 'failure', userType: string): void {
    this.authAttempts?.inc({ method, status, user_type: userType });
  }

  public recordMailOperation(operation: 'list' | 'read' | 'send', status: 'success' | 'failure', userId: string): void {
    this.mailOperations?.inc({ operation, status, user_id: userId });
  }

  public recordSmtpQueueSize(queueType: string, priority: string, size: number): void {
    this.smtpQueueSize?.set({ queue_type: queueType, priority }, size);
  }

  public recordSmtpProcessingTime(queueType: string, status: 'success' | 'failure', durationSeconds: number): void {
    this.smtpQueueProcessingTime?.observe({ queue_type: queueType, status }, durationSeconds);
  }
  ...
  public recordDmarcReport(domain: string, policy: string, disposition: string): void {
    this.dmarcReports?.inc({ domain, policy, disposition });
  }

  public recordTlsRptReport(domain: string, policy: string, result: string): void {
